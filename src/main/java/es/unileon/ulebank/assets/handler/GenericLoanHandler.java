		...
		if (loanID.length == 6) {
			...
			if (loanID[0].equals("LN") || loanID[0].equals("MG")) {
				try {
					...
					throw new MalformedHandlerException(
							"The month or year is not valid");
				}
				...
				if (!countryCodeExist) {
					throw new MalformedHandlerException(
							"Country code not exist.");
				}

				if (!(loanID[4].length() == 5)) {
					throw new MalformedHandlerException(
							"The length of the random characters must be 5.");
				}

				try {
					int checkDigit = Integer
							.parseInt(String.valueOf(loanID[5]));
					...
					throw new MalformedHandlerException(
							"The check digit is not valid, must be a number between 0 and 9.");
				}
